...
class AddTaskViewController: UIViewController {
    ...
    @IBAction func addTaskDidTouch(_ sender: Any) {
        ...
        let toDoItems = ToDoItem(context : context)
        toDoItems.completionDate = completionDate
        toDoItems.name = taskName
        toDoItems.details = taskDetails
        toDoItems.isComplete = false
        toDoItems.startDate = Date()
        
        (UIApplication.shared.delegate as! AppDelegate).saveContext()
        NotificationCenter.default.post(name: Notification.Name.init("AddTask"), object: nil, userInfo: nil)
        //(UIApplication.shared.delegate as! AppDelegate).saveContext()
        self.tabBarController?.selectedIndex = 0
        dismiss(animated: true, completion: nil)
